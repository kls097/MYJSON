<template>
  <div id="app">
    ...
    <TablePicker
      ...
      @select="handleTablePickerSelect"
    ...
    <template v-else>
      ...
      <ToolbarActions
        ...
        :table-candidate-count="tableCandidateCount"
...
// 检测是否可以打开表格视图（JSON有效即可，内部会智能检测数组）
const canOpenTable = computed(() => {
  if (!currentJson.value || !currentJson.value.trim()) return false
  try { JSON.parse(currentJson.value); return true } catch { return false }
})

// 检测到的表格候选数量（供工具栏显示）
const tableCandidateCount = computed(() => {
  if (!canOpenTable.value) return 0
  return detectTableCandidates(currentJson.value).length
})

// 当前视图模式（统一 viewMode 和 showTableMode）
const currentViewMode = computed(() => {
  if (showTablePicker.value) return 'table-picker'
...
const handleOpenTable = () => {
  try {
    const candidates = detectTableCandidates(currentJson.value)
    
    if (candidates.length === 0) {
      alert('未找到可表格化的数组数据\n\n提示：需要包含至少 2 个元素的对象数组')
      return
    }
    
    if (candidates.length === 1) {
      // 只有一个候选，直接进入表格
      tableData.value = candidates[0].data
      tableCandidatePath.value = candidates[0].path
      showTableMode.value = true
      return
    }
    
    // 多个候选，展示选择面板
    tableCandidates.value = candidates
...
// 从选择面板确认后进入表格
const handleTablePickerSelect = (candidate) => {
  tableData.value = candidate.data
  tableCandidatePath.value = candidate.path
...
const handleTableApply = (newData) => {
  ...
  const updated = setValueByPath(parsed, tableCandidatePath.value, newData)
